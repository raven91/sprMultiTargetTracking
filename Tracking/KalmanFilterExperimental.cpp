	...
	for (iter_trj_outer = trajectories.begin(); iter_trj_outer != trajectories.end(); ++iter_trj_outer)
	{
		...
		for (iter_trj_inner = trajectories.begin(); iter_trj_inner != trajectories.end(); ++iter_trj_inner)
		{
			...
			if ((Ti_e - Tj_b >= 0) && (Ti_e - Tj_b <= tau))
			{
				...
				cost_matrix[first_trj_idx][second_trj_idx] = CountCostMatrixElementIntersection(iter_trj_outer, iter_trj_inner, Ti_e, Tj_b);
			...
			if (Tj_e - Ti_b >= 0 && Tj_e - Ti_b <= tau)
			{
				...
				cost_matrix[first_trj_idx][second_trj_idx] = CountCostMatrixElementIntersection(iter_trj_inner, iter_trj_outer, Tj_e, Ti_b);
	...
	int tau)
{
	...
	for (int i = 0; i < costs.size(); ++i)
	{
		...
		if (Tj_b - Ti_e > 0 && Tj_b - Ti_e <= delta)
		{
			...
			PerformTrajectoryContinuation(outer_trajectory_iter, inner_trajectory_iter, outer_timestamps_iter, inner_timestamps_iter, s);
		...
		{
			std::cout << "At " << timestamps.find(max_idx)->second[0] << "th second " << "beginning trajectory index = " << outer_trajectory_iter->first;
			trajectories.erase(outer_trajectory_iter);
		}
		outer_trajectory_iter = trajectories.find(max_idx);
		if (outer_trajectory_iter != trajectories.end())
		{
			std::cout << " | | ending trajectory index = " << outer_trajectory_iter->first << "and they are now connected" << std::endl;
			trajectories.erase(outer_trajectory_iter);
		}
